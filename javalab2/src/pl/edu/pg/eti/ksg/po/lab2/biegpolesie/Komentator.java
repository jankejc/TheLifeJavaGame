...
class Komentator {
    ...
    public void relacjonuj(int nrElementu, Uczestnik u, DziedzinaZadania dz)
    {
        String slowo_uczestnik = odmienUczestnik(u);
        slowo_uczestnik = slowo_uczestnik.substring(0, 1).toUpperCase() + slowo_uczestnik.substring(1);
        mow(slowo_uczestnik+" "+u+" zmaga się z zadaniem z dziedziny "+dz+". Element nr "+nrElementu+" trasy.");
    }
    
    public void relacjonujGdyUczestnikWszedlNaMete(Uczestnik u, int miejsce)
    {
        switch(miejsce)
        {
            case 1:
                ...
                break;
            case 2:
                mow("Drugie miejsce na podium ma "+u);
                break;
            case 3:
                mow("Jako "+odmienTrzeci(u)+" zjawia się "+u);
                break;
            default:
                mow(u+" jest na mecie");
                break;
    ...
    private String odmienTrzeci(Uczestnik u) {
        String slowo_pierwszy = "trzeci";
        if(u instanceof Czlowiek && Czlowiek.Plec.KOBIETA.equals(((Czlowiek)u).getPlec()))
            slowo_pierwszy = "trzecia";
        return slowo_pierwszy;
    }
